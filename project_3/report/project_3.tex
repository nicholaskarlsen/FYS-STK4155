...
%% For the considered example cases, NN were not competetive with standard methods.
%% Particularly, for eigenproblem it seems like a bad approach if the goal is the eigenpairs themselves; RNNs, on the other hand, may be better suited. That is after all the context of Yi-etal. For our examples RNN's (or forward-euler) would probably be overkill, but at higher dimensionality one might expect better scaling.
