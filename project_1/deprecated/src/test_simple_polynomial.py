...
plt.figure()
plt.plot(np.log10(lambdas), lasso_lamb_deg_mse[1])
plt.plot(np.log10(lambdas), lasso_lamb_deg_mse[3])
plt.plot(np.log10(lambdas), lasso_lamb_deg_mse[6])
plt.plot(np.log10(lambdas), lasso_lamb_deg_mse[9])
plt.plot(np.log10(lambdas), lasso_lamb_deg_mse[12])
plt.title('selected degrees lasso lambda vs mse')
plt.show()

plt.figure()
plt.plot(np.log10(lambdas), ridge_lamb_deg_mse[1])
plt.plot(np.log10(lambdas), ridge_lamb_deg_mse[3])
plt.plot(np.log10(lambdas), ridge_lamb_deg_mse[6])
plt.plot(np.log10(lambdas), ridge_lamb_deg_mse[9])
plt.plot(np.log10(lambdas), ridge_lamb_deg_mse[12])
...
clf_best_lasso = skl.Lasso(alpha=best_lasso_lambda[max_degree-1],fit_intercept=False).fit(X_train_scaled,z_train)
print('betas from lasso for highest degree in test, and best lambda for that degree')
print(clf_best_lasso.coef_)

clf_best_ridge =  skl.Ridge(alpha=best_ridge_lambda[max_degree-1],fit_intercept=False).fit(X_train_scaled,z_train)
