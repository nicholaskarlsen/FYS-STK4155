...
\section{Theory}
    ...
    \subsection{Linear Regression}
        ...
        \subsubsection{Ridge Regression}
            ...
            \end{equation}
            Similar for what we did for OLS, we take the partial derivative wrt. $\pmb \beta$, in which the only difference recides in the last term, which gives
            \begin{equation}
                \pdv{\pmb \beta} \lambda \pmb\beta^T\pmb\beta = 2\lambda\pmb\beta
            \end{equation}
            if we again assert that the optimal $\pmb\beta$ is given by $\partial_{\pmb\beta}C_R(\pmb\beta)$ and rearange we get
            \begin{equation}
